...
identity on the trailing `n − d` qubits. This is the canonical `D_{c·x_S}` with `|S| = d`.

## Main results

* `topProj_inBW_iff` — the **sub-coordinate conductor**: the leading-`d` projector
 scaled by `λ^j` lies in `BW_n` iff `2d ≤ j`. For `d = n` it specialises to
 `corner_inBW_iff` (since `topProj n n = bwCorner n`).
