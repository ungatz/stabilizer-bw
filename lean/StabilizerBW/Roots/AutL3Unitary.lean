...
The unitary version of the Kliuchnikov–Schönnenbeck 2024 converse at one
qubit: every unitary integral automorphism of the level-3 Barnes–Wall lattice
`L₃` is a phased Clifford.

We work over the ring `R = ℤ[ζ₈]`, modelled as the structure `Z8` of
quadruples `(a,b,c,d)` standing for `a + b·ζ + c·ζ² + d·ζ³` with `ζ⁴ = -1`.
Here `ζ = ζ₈` is a primitive 8-th root of unity, `i = ζ²`, `√2 = ζ - ζ³`.
`L₃ = {(a,b) ∈ R² : (1+i) ∣ a + b}`.

The headline statements are

 `det_root_of_unity` — a unitary integral matrix has `det ∈ ⟨ζ₈⟩`;
 `finite_integral_su2_aut_L3` — the integral special-unitary
 lattice-automorphism group is finite, of cardinality exactly 16;
 `aut_L3_unitary_is_phased_clifford` — every unitary integral automorphism
 of `L₃` is a phase `ζ₈^k` times an integral Clifford.

## A note on the count: this is not the standard K–S theorem

The integral-over-`ℤ[ζ₈]` unitary `Aut(L₃)` is the monomial Clifford subgroup
of order 16 (projective image 8) — a *strict subgroup* of the full single-qubit
Clifford group that K–S 2024 enumerate over `ℤ[1/√2, i]` (order 24,
projective). The reason is integrality. A unitary matrix over `ℤ[ζ₈]` is
forced to be monomial with root-of-unity entries (`unitary_is_monomial`),
because each Hermitian row norm is a sum of integer squares equal to 1. The
Hadamard gate `(1/√2)·[[1,1],[1,-1]]` has entries `±1/√2`, which are not
algebraic integers and so do not lie in `ℤ[ζ₈]`; Hadamard — the unique
non-monomial Clifford generator — is therefore excluded, and the integral
count collapses from 24 to 16 (projective 8).
...
The integral unitary automorphism group itself has order 64 (8 phases times
the monomial Clifford subgroup of order 8; here the linear monomial Clifford
group `cliffFinset` has order 32), its projective image has order 8, and the
integral special-unitary (`det = 1`) part has order 16 — not 24
(`finite_integral_su2_aut_L3`). The unitary converse itself remains true:
every unitary integral `L₃`-automorphism is a phased (monomial) Clifford.
-/
...
/-! ## Determinant of a unitary integral matrix is a root of unity -/

/-- Every unitary integral matrix over `ℤ[ζ₈]` has determinant equal to a power
of `ζ₈`. (The `MapsToL` hypothesis is not needed for the result and is omitted;
cf. `det_root_of_unity'` for the stated form.) -/
...
/-- Same result with the (unnecessary) automorphism hypothesis carried, for
downstream uses that already assume it. -/
...
The full projective single-qubit Clifford group `≅ S₄` has order `24` (K–S 2024).
Over the integer ring `ℤ[ζ₈]` the Hadamard gate is **not** integral, so it is
excluded and the count collapses to `16` (projective image `8`). See the
module docstring and the companion `AutL3HalfSqrt2.lean` for the lift to `24`. -/
