...
/-!
# Towards the general-`n` Möbius closed form (R10 · T2)

The round-R9 work established, kernel-verified on the full 32-case table at `n ≤ 5`
(see the companion module `MultimonomialClosedForm`),
form
...
The R10 target is to lift this to a general-`n` theorem.  The proof route flagged in the
prompt is the Barnes–Wall `(u, u+v)` free-module recursion: splitting the coordinate set
as `{1,…,n−1} ⊔ {n}` the phase vector `w_e ∈ ℤ[ζ₈]^{2ⁿ}` decomposes as
...
It then closes **steps 1–3** of the round's attack plan over the canonical Barnes–Wall
...
## A correction to the prompt's statement of the lemma

The prompt writes the split lemma as
...
(`Roots/BWn.lean`, `Roots/LowerBoundAllN.lean`), namely steps 1–3 of the round's attack
...
Sign note: the prompt writes the inner half as `w^{(1)} − w^{(0)}`; the canonical lattice
is closed under negation, so we use the equivalent `w^{(0)} − w^{(1)} = bwSub n D.1 D.2`
matching the direction of `Roots.inBW_succ_iff`.
-/
