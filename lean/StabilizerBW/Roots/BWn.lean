...
This module gives the inductive lattice recursion. The main result is the
all-`n` geometric a-priori ceiling
...
theorem bwSub_bwId_bwT (n : ℕ) :
 ...
 have h1 : bwSub m (bwId m) (bwId m) = bwSmul m lam (bwZero m) := by
 rw [bwSub_self, bwSmul_bwZero]
 rw [h1]
