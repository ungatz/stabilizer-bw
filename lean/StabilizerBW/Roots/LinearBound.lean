...
This is the audit fallback called for in the round prompt; the general-`n` upper bound
would require the full `BW_n` tower (not yet mechanized), but every tabulated case here
satisfies the linear formula *exactly* (so in particular the `≤` upper bound holds).
