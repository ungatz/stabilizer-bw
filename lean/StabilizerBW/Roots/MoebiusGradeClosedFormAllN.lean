...
theorem mob_castSet_congr {n : ℕ} (h : Finset (Fin (n + 1)) → Z8)
  ...
  refine' Finset.sum_bij ( fun V _ => Finset.univ.filter fun i => Fin.castSucc i ∈ V ) _ _ _ _ <;> simp_all +decide [ Finset.subset_iff ];
  · intro a ha x hx; specialize ha hx; unfold castSet at ha; aesop;
  · intro a₁ ha₁ a₂ ha₂ h; ext x; by_cases hx : x = Fin.last n <;> simp_all +decide [ Finset.ext_iff ] ;
    · exact iff_of_false ( fun hx' => by simpa [ hx ] using last_not_mem_castSet U <| ha₁ hx' ) ( fun hx' => by simpa [ hx ] using last_not_mem_castSet U <| ha₂ hx' );
    · cases x using Fin.lastCases <;> aesop;
  · intro V hV; use Finset.image ( fun x => Fin.castSucc x ) V; simp_all +decide [ Finset.ext_iff, castSet ] ;
  · intro a ha; rw [ show a = Finset.image ( Fin.castSucc ) ( Finset.filter ( fun i => Fin.castSucc i ∈ a ) Finset.univ ) from ?_ ] ; simp +decide [ Finset.card_image_of_injective, Function.Injective, * ] ;
    · convert congr_arg _ ( hyp _ _ ) using 2;
      · exact congr_arg _ ( by ext; simp +decide [ castSet ] );
      · rw [ card_castSet ];
      · intro x hx; specialize ha ( Finset.mem_filter.mp hx |>.2 ) ; unfold castSet at ha; aesop;
    · ext x; simp [castSet] at *; (
      exact ⟨ fun hx => by obtain ⟨ y, hy, rfl ⟩ := ha hx; exact ⟨ y, by simpa using hx, rfl ⟩, by rintro ⟨ y, hy, rfl ⟩ ; exact hy ⟩ ;);
