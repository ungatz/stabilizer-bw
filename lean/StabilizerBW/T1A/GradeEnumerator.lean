...
/-!
# T1A — the pure-linear Barnes–Wall grade enumerator
...
/-- The constant in the ANF contributes a factor `8` (any of the
8 ℤ/8 values, all with grade contribution `0`). -/
theorem constant_GF (z : ℤ) : (∑ _c : ZMod 8, z ^ (0 : ℕ)) = 8 := by simp

/-- Per-linear-monomial generating function: `4` even-coefficient
...
/-- **Headline (factored form).** The pure-linear grade generating
...
/-- **Headline closed form.** `G_m(z) = 8 · 4^m · (1 + z)^m`. -/
