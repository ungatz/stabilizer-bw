...
Combining the uniform `τ`-pushforward , the grade = parity-count identity
and the binomial parity-vector enumerator gives the closed-form
...
theorem sum_oddIndic_eq {m : ℕ} (f : Fin m → ZMod 8) :
    (∑ i, ReedMuller.oddIndic (f i)) = (Finset.univ.filter (fun i => parityBit (f i) = true)).card := by
  rw [Finset.card_filter]
  apply Finset.sum_congr rfl
  intro i _
  unfold ReedMuller.oddIndic parityBit
...
theorem weight_count (m k : ℕ) :
    (Finset.univ.filter (fun f : Fin m → ZMod 8 => (∑ i, ReedMuller.oddIndic (f i)) = k)).card
...
theorem grade_fiber_card {m : ℕ} (k : ℕ) :
    (Finset.univ.filter (fun P : ReedMuller.LinPhase m => ReedMuller.gradeOf P = k)).card
      = 8 * 4 ^ m * Nat.choose m k := by
  classical
  have hset : (Finset.univ.filter (fun P : ReedMuller.LinPhase m => ReedMuller.gradeOf P = k))
      = (Finset.univ : Finset (ZMod 8)) ×ˢ
          (Finset.univ.filter (fun f : Fin m → ZMod 8 => (∑ i, ReedMuller.oddIndic (f i)) = k)) := by
    ext ⟨c, f⟩
