...
of a linear Barnes–Wall phase polynomial and the symmetric chain's measurement-side
...
parity-tag pushforward uniform.
-/
...
/-- The per-coordinate odd-indicator of `ReedMuller.oddIndic` is the `ℕ`-valued
parity bit. -/
theorem oddIndic_eq_toNat (c : ZMod 8) : ReedMuller.oddIndic c = (parityBit c).toNat := by
