...
/-!
# T8 — refutation hook and the exclusion of the r1 counterexample
...
falsification interface (Layer 73/77 pattern) — a publishable separation would instantiate it, but
the headline proves it can never be instantiated on `PhysicalDensity`.

We also record explicitly that the **r1 counterexample** (`ρ_X = 1000` at `m = 1`), which falsified
the unrestricted r1 carrier, is *excluded* by the physical-density encoding: no `PhysicalDensity`
...
/-- **The r1 counterexample is excluded.**  No physical density operator can carry an arbitrarily
large coefficient such as the r1 falsifier `ρ_X = 1000`: every coefficient satisfies `|χ_P| ≤ 1`. -/
theorem r1_counterexample_excluded {m : ℕ} (ρ : PhysicalDensity m) (P : PauliIdx m) :
    |ρ.1 P| ≤ 1 := ρ.2.1 P

/-- Consequently no physical density operator realises the r1 falsifier's coefficient value. -/
