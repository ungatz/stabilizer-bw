...
(angle 4.2 of the development), where the recursive coordinate structure is
genuine, and transport the results to `Fin (2 ^ m) → ZMod 2` along an
explicit linear equivalence `ptEquiv` (a relabelling of evaluation points).
