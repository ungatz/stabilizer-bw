...
structure `CSSCode` (no quantum stabiliser formalism is needed — see angle 4.4
of the original), and build the Reed–Muller-pair member `CSSCode.ofRMPair` whose
quantum parameters `[[n, k, d]]` are
`n = 2^m`, `k = ∑_{r₂ < i ≤ r₁} C(m,i)`, `d = 2^(m-r₁)`.
...
we record the X-distance, which is the quantity the structural strawman's parameter formula
names.)
-/
