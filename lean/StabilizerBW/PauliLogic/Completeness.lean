/-
# Completeness of PL_n

The converse of soundness: every valid stabilizer entailment `Γ ⊢ Q` has a
derivation in `PL_n`.

The argument is a trace identity. Let `gen Γ a` be the signed subset product
of the generators selected by `a : Fin Γ.length → Bool`; the closure `⟨Γ⟩` is
`{ gen Γ a | a }`. The basis trace `ptrace P` is `2^n` on `+I`, `-2^n` on
`-I`, and `0` on every other Pauli word, so the averaged operator
`stabSum Γ := Σ_a pauliAction (gen Γ a)` has nonzero trace exactly when
`-I ∉ ⟨Γ⟩`. In that case `stabSum Γ` is a nonzero element of the codespace,
hence fixed by `pauliAction Q` whenever `Γ ⊢ Q` is valid. Taking traces of
`Q · stabSum = stabSum` gives
`#{a : gen a = Q} = #{a : gen a = +I} + #{a : gen a = -Q} ≥ 1`, so `Q ∈ ⟨Γ⟩`.
That is exactly derivability.
-/
