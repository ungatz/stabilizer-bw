...
/-!
# T2 — Spectral-gap reuse (Levin–Peres–Wilmer convention)

This file re-exports the Layer 92 spectral-gap carrier
`ParityChainBWGradeMixing.SpectralGapCarrier.LaRacuenteSpectralGapBound`
and records its specialisation at the symmetric-transition point `p = 1/2`,
where LaRacuente's gap `4(1-p)/m` becomes `2/m`.
...
theorem lpw_gap_at_pHalf (m : ℕ) : laracuenteGap (1 / 2) m = 2 / (m : ℝ) :=
...
theorem lpw_gap_pos {m : ℕ} (hm : 1 ≤ m) : 0 < laracuenteGap (1 / 2) m :=
  laracuenteGap_pos (by norm_num) hm

end BWParityChainMixingTime.SpectralGapReuse
