...
This repository tells that story in three layers. Lean 4 sources for every theorem live in [`lean/`](lean/), with machine-checked proofs. A Haskell library that runs everything on concrete data is in [`haskell/`](haskell/). A chapter-by-chapter prose tour of the mathematics, with explicit citations for what is in the literature and what is new here, is in [`narrative/`](narrative/).

The repository is standalone. Nothing in the prose cites material outside this tree.
...
| [`narrative/07-r11-transport.md`](narrative/07-r11-transport.md) | Direct closure of the Clifford-transport step at $n = 2, 3$ |
...
The Haskell library is plain base + containers, no package manager required.
...
The Lean 4 sources live in [`lean/`](lean/) organised by topic. They compile against a recent Lean 4 toolchain with Mathlib; the parent project that hosts them carries the exact Lake configuration. See [`lean/README.md`](lean/README.md) for the per-file map.
