...
the **totally ramified prime above 2**: the field norm $N(\lambda) = 2$ and $\lambda^4 = -2\zeta_8 + 6\zeta_8^2 - 4\zeta_8^3$ is an associate of $2$. The single equality $(\lambda)^4 = (2)$ encodes everything about how the lattice tower handles the new resource (the $T$ gate) introduced by Clifford+$T$.
...
```math
g(M) = \min\{\, k : \lambda^k \cdot M \cdot L_3 \subseteq L_3\,\}.
```
...
A **diagonal character** on $n$ qubits is an operator $D_e \cdot |x\rangle = \zeta_8^{e(x)}|x\rangle$ for some function $e \colon \mathbb{F}_2^n \to \mathbb{Z}/8\mathbb{Z}$. Write the function as a multilinear polynomial $e(x) = c \cdot x_S$ where $S \subseteq \{1, \dots, n\}$ is its support and $c \in \mathbb{Z}/8\mathbb{Z}$ its eighth-root coefficient. Write $d = |S|$ for the cardinality of $S$ and $\nu_2(c)$ for the 2-adic valuation of $c$ taken mod 8. For these **single-monomial** characters there is a closed form for the grade at the level-$n$ Barnes–Wall lattice:

```math
g(D_{c\,x_S}) = \max\bigl(0,\ 2d - 2^{\nu_2(c)}\bigr).
```

Examples (the second column shows $d$, the third $c$, the fourth the formula, the fifth the grade):
...
The upper-bound half — $g(D_{c\,x_S}) \le \max(0, 2d - 2^{\nu_2(c)})$ — is proved at every $n$ by factoring the character through a projector. The lower-bound half is proved at every $n$ for the maximal monomial ($d = n$) and every $\nu_2(c) \in \{0, 1, 2\}$, by exhibiting a witness vector. For strict-subset monomials ($d < n$) at general $n$, the linear closed form is verified by direct kernel computation at $n \le 4$ and routed for general $n$ through a structured induction.

## The $T$-count corollary

For a word $w$ over the integral generator set $\{S, X, T\}$ on a fixed number of qubits, the grade of the denoted operator is at most the number of $T$ letters in $w$, with equality when $w = T$. This is the **$T$-count lower bound**: a circuit cannot denote a gate of grade $g$ using fewer than $g$ instances of $T$.
...
* The two-qubit Barnes–Wall lattice $\mathrm{BW}\_{2}$ with decidable membership, and the kernel-verified diagonal table $g(CZ) = 0$, $g(T \otimes I) = 1$, $g(CS) = 2$, $g(T \otimes T) = 2$, $g(cT) = 3$ — [`../lean/Arithmetic/Roots/BW2.lean`](../lean/Arithmetic/Roots/BW2.lean), `Tensor.lean`.
* Three- and four-qubit grades, including the kernel-verified $g(ccT) = 5$ and $g(cccT) = 7$ — `BW3.lean`, `BW4.lean`.
* The general-$n$ upper bound $g(D_{c\,x_S}) \le \max(0, 2d - 2^{\nu_2(c)})$ — `UpperBoundAllN.lean`.
