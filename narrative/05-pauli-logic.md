...
Every derivation in $\mathsf{PL}_n$ normalises to a cut-free derivation in linear time. The cut-free derivations of a literal $Q$ from $\Gamma$ are in bijection with **subset-product certificates**: subsets $T \subseteq \{1, \dots, |\Gamma|\}$ such that $Q = \prod_{k \in T} \Gamma_k$ (the multiplication evaluated in any order, with signs tracked by the $i$-exponent).
...
The arithmetic-side reading — every (Mul) step is one factor of $\varphi$ in the lattice semantics — is the bridge to the grade story of [`06-grade.md`](06-grade.md): a derivation of length $\ell$ in $\mathsf{PL}_n$ corresponds to a coordinate move that loses $\ell$ factors of $\varphi$. This bridge is why the logical-lattice theorem of [`03-logical-lattice.md`](03-logical-lattice.md) supplies the lattice semantics of $\mathsf{PL}_n$, with $\llbracket \Gamma \vdash P \rrbracket$ a containment of constraint sublattices.
